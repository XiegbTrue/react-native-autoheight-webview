...
public class RNWebViewModule extends ReactContextBaseJavaModule implements ActivityEventListener {
    ...
    @VisibleForTesting
    public static final String REACT_CLASS = "RCTAutoHeightWebView";
    ...
    private final static int REQUEST_SELECT_FILE_TENCENT_X5 = 1003;
    ...
    //For tencent X5
    public void openFileChooseProcess(ValueCallback<Uri> uploadMessage,ValueCallback<Uri[]> uploadMessageArr) {
        this.mUploadMessage = uploadMessage;
        this.mUploadMessageArr = uploadMessageArr;

        Intent intent = new Intent(Intent.ACTION_GET_CONTENT);
        intent.addCategory(Intent.CATEGORY_OPENABLE);
        intent.setType("*/*");
        getCurrentActivity().startActivityForResult(Intent.createChooser(intent, "选择图片"), REQUEST_SELECT_FILE_TENCENT_X5);
    }
    ...
    public boolean startFileChooserIntent(ValueCallback<Uri[]> filePathCallback, Intent intentChoose) {
        ...
        Activity currentActivity = getCurrentActivity();
        Log.e(REACT_CLASS,currentActivity.getClass().toString());
        if (currentActivity == null) {
            Log.w(REACT_CLASS, "No context available");
            return false;
        }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_SELECT_FILE_LEGACY) {
            ...
            mUploadMessageArr = null;
        } else if(requestCode == REQUEST_SELECT_FILE_TENCENT_X5 && Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP){
